...
public class MotionProfileGenerator {
    ...
    protected static MotionProfile generateFlippedProfile(
            ...
            final MotionProfileGoal goal_state,
            final MotionState prev_state
    ) {
        final MotionProfile profile = generateProfile(constraints, goal_state.flipped(), prev_state.flipped());

        for (final MotionSegment s : profile.segments()) {
    ...
    public synchronized static MotionProfile generateProfile(
        ...
        MotionState start_state = new MotionState(prev_state.t(), prev_state.pos(),
                signum(prev_state.vel()) * min(abs(prev_state.vel()), constraints.max_abs_vel()),
                signum(prev_state.acc()) * min(abs(prev_state.acc()), constraints.max_abs_acc()));
        ...
        if (start_state.vel() < 0.0 && delta_pos > 0.0) {
            final double stopping_time = abs(start_state.vel() / constraints.max_abs_acc());
        ...
        final double min_abs_vel_at_goal = Math.sqrt(abs(min_abs_vel_at_goal_sqr));
        ...
        if (min_abs_vel_at_goal_sqr > 0.0 && min_abs_vel_at_goal > (goal_state.max_abs_vel() + goal_state.vel_tolerance())) {
            ...
            if (goal_state.completion_behavior() == CompletionBehavior.VIOLATE_MAX_ABS_VEL) {
                ...
                if (abs(delta_pos) < goal_state.pos_tolerance()) {
                ...
                max_acc = abs(goal_vel * goal_vel - start_state.vel2()) / (2.0 * delta_pos);
            } else {
        ...
        goal_vel = min(goal_vel, max_abs_vel_at_goal);
        ...
        final double v_max = min(constraints.max_abs_vel(), sqrt((start_state.vel2() + goal_vel * goal_vel) / 2.0 + delta_pos * max_acc));
        ...
        final double distance_decel = max(0.0, (start_state.vel2() - goal_vel * goal_vel) / (2.0 * constraints.max_abs_acc()));
        final double distance_cruise = max(0.0, goal_state.pos() - start_state.pos() - distance_decel);
